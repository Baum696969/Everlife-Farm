...
export default function FarmerPanel({
  ...
  onBuyFarmer, onUpgradeFarmer, onUpgradeEndlessStat, onGiveSeeds, onCollect,
    ...
    return (
      <div className="space-y-3">
        ...
        <div className="space-y-1.5">
          <h3 className="text-xs font-bold">📦 Inventar ({farmer.inventory.length}/3 Typen, {farmer.inventory.reduce((s, i) => s + i.amount, 0)} Seeds)</h3>
        ...
        {/* Active slots */}
        <div className="space-y-1.5">
          <h3 className="text-xs font-bold">🌱 Aktive Aufträge ({farmer.slots.length}/{stats.slots})</h3>
            ...
            {endlessStatDefs.map(def => {
              ...
              return (
                <div key={def.key} className="flex items-center justify-between p-2 bg-muted rounded-lg text-xs">
                  <div>
                    <p className="font-semibold">{def.emoji} {def.name} <span className="text-muted-foreground">Lv. {currentLevel}</span></p>
